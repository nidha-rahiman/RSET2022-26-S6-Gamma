...
# Patients P01–P12
kurtosis_data = {f"P{p:02d}": [] for p in range(1, 13)}
    ...
    for file_num in range(1, 6):
        for segment_num in range(1, 56):
            ...
            if os.path.exists(file_path):
                ...
                emg_values = df["Channel_1"].values  # Updated column name
                kurt = kurtosis(emg_values)
...
kurt_df = pd.DataFrame.from_dict(kurtosis_data, orient="index", columns=columns)
kurt_df.to_csv("kurtosis.csv", index=True)

print("✅ kurtosis.csv saved.")
