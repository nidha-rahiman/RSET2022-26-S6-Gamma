...
# Patients P01–P12
skew_data = {f"P{p:02d}": [] for p in range(1, 13)}
    ...
    for file_num in range(1, 6):
        for segment_num in range(1, 56):
            ...
            if os.path.exists(file_path):
                ...
                sk = skew(emg_values)
                skew_data[patient_id].append(sk)
            else:
                print(f"Missing: {file_name}")
                skew_data[patient_id].append(None)

# Create column names: F01S01 to F05S55
columns = [f"F{f:02d}S{s:02d}" for f in range(1, 6) for s in range(1, 56)]

# Create and save DataFrame
skew_df = pd.DataFrame.from_dict(skew_data, orient="index", columns=columns)
skew_df.to_csv("skewness.csv", index=True)

print("✅ skewness.csv saved.")
